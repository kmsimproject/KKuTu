...
if(CPU >= 2 && CF){
	console.log(`Max Channel Limit is 1 when CloudFlare is Enabled.`);
}
if(Cluster.isMaster){
	var channels = {}, chan;
	var i;
	...
	for(i=0; i<CPU; i++){
		chan = i + 1;
		channels[chan] = Cluster.fork({ SERVER_NO_FORK: true, KKUTU_PORT: Const.MAIN_PORTS[SID] + (CF ? 30 : 416) + i, CHANNEL: chan });
	}
	Cluster.on('exit', function(w){
		...
		channels[chan] = Cluster.fork({ SERVER_NO_FORK: true, KKUTU_PORT: Const.MAIN_PORTS[SID] + (CF ? 30 : 416) + (chan - 1), CHANNEL: chan });
	});
